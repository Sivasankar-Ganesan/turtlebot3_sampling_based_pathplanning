...
Map_manager::Map_manager() {
  image = imread("/home/username/catkin_ws/src/turtlebot3_sampling_based_pathplanning/maps/map.png",cv::IMREAD_GRAYSCALE);

  if (!image.data) {
    std::cout << "No Data at the provided map location. Please check the file path." << std::endl;
    exit(EXIT_FAILURE);
  }
...
// returns the Cspace
std::vector<std::vector<int> > Map_manager::getCfree() { return Cfree; }
...
std::vector<float> Map_manager::computeGridCoordinate(
...
std::vector<double> Map_manager::computeDistanceCoordinate(
    std::vector<float> position) {
